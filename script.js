document.addEventListener('DOMContentLoaded', () => {
    ...
    async function applyTheme(themeName) {
        ...
        if (user) {
            ...
            // Fallback to localStorage if user is not logged in (optional)
    ...
    async function fetchData() {
        const { data: { user } } = await supabaseClient.auth.getUser();
        if (!user) {
             appState = { incomes: [], expenses: [] }; // Clear data if no user
             renderAll();
             return;
        }
    ...
    // === INITIALIZATION ===
    // loadTheme is now called by onAuthStateChange, so no initial call needed here.
    ...
    function renderExpenseChart(){const categoryTotals={};appState.expenses.forEach(expense=>{const monthlyAmount=calculateMonthlyTotal([expense]);if(!categoryTotals[expense.category]){categoryTotals[expense.category]=0}
    categoryTotals[expense.category]+=monthlyAmount});const labels=Object.keys(categoryTotals);const data=Object.values(categoryTotals);if(expenseChartInstance){expenseChartInstance.destroy()}
    if(labels.length===0)return;const ctx=expenseChartCanvas.getContext('2d');expenseChartInstance=new Chart(ctx,{type:'pie',data:{labels:labels,datasets:[{label:'Expenses by Category',data:data,backgroundColor:['#3498db','#e74c3c','#9b59b6','#f1c40f','#2ecc71','#1abc9c','#e67e22','#95a5a6'],hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top'}}}})}
