document.addEventListener('DOMContentLoaded', () => {
    ...
    function showIncomeModal(incomeId) {
        ...
        onSave = async () => {
            ...
            let error;
            if (isEditMode) { ({ error } = await supabaseClient.from('incomes').update(formItem).eq('id', incomeId)); } 
            else { ({ error } = await supabaseClient.from('incomes').insert(formItem)); }
    ...
    function showExpenseModal(expenseId) {
        ...
        onSave = async () => {
            ...
            let error;
            if (isEditMode) { ({ error } = await supabaseClient.from('expenses').update(formItem).eq('id', expenseId)); } 
            else { ({ error } = await supabaseClient.from('expenses').insert(formItem)); }
