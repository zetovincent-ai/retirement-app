document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveSettingsBtn = document.getElementById('save-settings-btn'); // New selector

    // === FUNCTIONS ===

    // --- THEME FUNCTIONS ---
    // Applies the theme visually but DOES NOT save to DB
    function applyThemeVisually(themeName) {
        document.body.dataset.theme = themeName;
        // Optionally update localStorage for immediate visual persistence if needed,
        // but DB is the source of truth on load.
        localStorage.setItem('sunflower-theme-preview', themeName);
    }
        ...
        if (user) {
            ...
            if (data && data.settings && data.settings.theme) {
                ...
                console.log(`Theme '${theme}' loaded from DB.`);
                 ...
                 console.log("No theme found in DB for user, using default.");
            }
        } else {
             // Fallback for logged-out users (using preview or default)
             theme = localStorage.getItem('sunflower-theme-preview') || 'default';
             console.log(`User not logged in, theme '${theme}' loaded from localStorage/default.`);
        }

        document.body.dataset.theme = theme; // Apply the authoritative theme
        if (themeSwitcher) {
    ...
    async function handleSaveSettings() {
        ...
        if (user) {
            ...
            if (error) {
                ...
                alert('Failed to save settings.'); // Provide user feedback
            } else {
                ...
                // Optionally close modal on successful save
                 setTimeout(() => { // Small delay for user to see 'Saving...'
                      closeSettingsModal();
                 }, 500);
            }
             // Re-enable button
            ...
            // Save locally if not logged in (optional)
            applyThemeVisually(selectedTheme);
            localStorage.setItem('sunflower-theme-local', selectedTheme); // Use a different key if needed
            closeSettingsModal();
        }
    }
    ...
    function closeSettingsModal() {
        ...
        // Optional: Revert visual theme to last saved state if changes weren't saved
        loadTheme();
    }
    ...
    supabaseClient.auth.onAuthStateChange(async (event, session) => {
        ...
        await loadTheme(); // Load theme *after* user status is known
        if (session) {
             fetchData();
        } else {
             appState = { incomes: [], expenses: [] };
             renderAll();
             // Theme reset on logout is handled by loadTheme's fallback
        }
    });

    toggleDashboardBtn.addEventListener('click', () => { mainContainer.classList.toggle('dashboard-expanded'); });

    // Update theme visually immediately on change, but don't save yet
    ...
    saveSettingsBtn.addEventListener('click', handleSaveSettings); // New listener
    ...
    // Initial theme load on page start is handled by onAuthStateChange


    // --- UNCHANGED FUNCTION DEFINITIONS ---
    async function fetchData(){console.log("Attempting to fetch data...");const{data:{user}}=await supabaseClient.auth.getUser();if(!user){console.log("No user logged in for fetchData, clearing local state.");appState={incomes:[],expenses:[]};renderAll();return}
    console.log("Fetching incomes and expenses for user:",user.id);const{data:incomes,error:incomesError}=await supabaseClient.from('incomes').select('*').eq('user_id',user.id);if(incomesError)console.error('Error fetching incomes:',incomesError);else appState.incomes=incomes||[];const{data:expenses,error:expensesError}=await supabaseClient.from('expenses').select('*').eq('user_id',user.id);if(expensesError)console.error('Error fetching expenses:',expensesError);else appState.expenses=expenses||[];console.log("Data fetch complete, rendering UI.");renderAll()}
    async function handleLogin(){console.log("Login initiated...");const{error}=await supabaseClient.auth.signInWithOAuth({provider:'github'});if(error)console.error('Error logging in:',error)}
    async function handleLogout(){console.log("Logout function called");const{error}=await supabaseClient.auth.signOut();if(error){console.error('Error logging out:',error)}else{console.log("Logout successful via function call.")}}
    function updateUserStatus(user){console.log("Updating user status UI for user:",user?user.email:'null');if(user){userStatus.innerHTML=`Logged in as ${user.email} <button id="logout-btn" class="btn-secondary">Logout</button>`;const logoutBtn=document.getElementById('logout-btn');if(logoutBtn){logoutBtn.removeEventListener('click',handleLogout);logoutBtn.addEventListener('click',handleLogout)}else{console.error("Logout button not found after update.")}}else{userStatus.innerHTML=`<button id="login-btn" class="btn-primary">Login with GitHub</button>`;const loginBtn=document.getElementById('login-btn');if(loginBtn){loginBtn.removeEventListener('click',handleLogin);loginBtn.addEventListener('click',handleLogin)}else{console.error("Login button not found after update.")}}}
